...
connection.connect((err) => {
  ...
  console.log(
    "Connectée en tant qu'id de thread numéro : " + connection.threadId
  );
});
  ...
  try {
    ...
    if (!user || !email || !password) {
      return res.status(400).send({
        ...
        errorCode: "REGISTER_MISSING_FIELDS",
      });
    }
    ...
    connection.query(emailQuery, [email], (err, results) => {
      if (err) {
        ...
        return res.status(500).send({
          ...
          errorCode: "REGISTER_EMAIL_CHECK_FAILED",
        });
      }
      ...
      if (results.length > 0) {
        return res.status(400).send({
          ...
          errorCode: "REGISTER_EMAIL_IN_USE",
        });
      }
      ...
      connection.query(userQuery, [user], (err, results) => {
        if (err) {
          ...
          return res.status(500).send({
            ...
            errorCode: "REGISTER_USERNAME_CHECK_FAILED",
          });
        }
        ...
        const insertQuery =
          "INSERT INTO User (user, email, password, token, isAdmin) VALUES (?, ?, ?, ?, ?)";
        connection.query(
          insertQuery,
          [user, email, hashedPassword, token, 0],
          (err, results) => {
            if (err) {
              console.error(err);
              return res.status(500).send({
            ...
            console.log(
              `User ${user} registered successfully with token ${token}`
            );
...
function randomToken() {
  ...
  const randomNumbers = Array.from({ length: position }, () =>
    Math.floor(Math.random() * 256)
  ).join("-");

  // Ajouter des éléments basés sur le timestamp pour plus d'entropie
  const timestampPart =
    randomString(8, allChars) + "-" + timestamp.toString(36);
